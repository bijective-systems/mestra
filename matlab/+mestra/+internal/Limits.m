...
%                               the 2^31 section 29 says to state
    ...
    methods (Static)
        ...
        %set  Change one limit and return what it was.
            ...
            if ~isnumeric(value) || ~isscalar(value) || ~isfinite(value) || ...
               value < 1
                error('mestra:limits', ...
                      ['the limit "%s" must be a finite number of ' ...
                       'at least 1'], name);
            end
            previous = state.(name);
        ...
        %defaults  The documented numbers above.
            s = struct('maxElements', 2147483648, ...
                       'maxDepth', 64, ...
                       'maxObjects', 200000, ...
                       'maxStringSize', 65536);
        end
        ...
        %   Section 29 makes this E41: an eager read of a dataset above
        %   the maximum the reader states.  A lazy read and a row-range
        %   read are not subject to it, because they never materialise
        %   the whole dataset.
            limit = mestra.internal.Limits.get('maxElements');
            if count > limit
                error('mestra:E41', ...
                      ['%s declares %s elements and this reader will ' ...
                       'materialise at most %d in one eager read ' ...
                       '(E41). Nothing was read. Raise the limit with ' ...
                       'mestra.limits if the file is that large.'], ...
